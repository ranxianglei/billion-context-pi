...
- **Window-scaled default (#398):** when this key is unset, sub-150K windows default the band to a third of the window. The kernel's fixed 50000 only makes sense at 150K+: below that, 50K of growth can never accumulate before the 75% force band takes over, leaving the soft cadence dead weight and compression driven entirely by the forced/emergency bands. With a 50K window the ~16.7K band starts soft-nudging at ~52% usage and re-arms every ~16.7K of growth — small-context sessions stay in a steady compress loop instead of whipsawing into forced compression. An explicit value always wins; set `50000` to restore the legacy fixed band.
- **Small-context recipe (~50K windows):** with #398 the compress side needs no knobs. To also absorb oversized single tool outputs early (instead of letting them ride until a compress), merge [PR #208](https://github.com/ranxianglei/billion-context-pi/pull/208) and set `{ "absorb": { "minToolTokens": 4000 } }` — the default 1000 (2% of a 50K window) also absorbs ordinary outputs; 4000 (~8%) targets only the genuinely large ones.
