...
| Key | Type | Default | Status | Description |
|-----|------|---------|--------|-------------|
| `absorb` | boolean \| object | `false` | 🟢 ACTIVE | `true` enables instant tool-result absorption with defaults; pass an object to tune. |
| `absorb.toolName` | string | `"absorb"` | 🟢 ACTIVE | Name of the absorb tool exposed to the model. |
| `absorb.minToolTokens` | number | `1000` | 🟢 ACTIVE | Tool results below this estimated size never trigger an absorption prompt. |
| `absorb.contextThresholdPct` | number \| string | `0` | 🟢 ACTIVE | Only prompt once context usage reaches this fraction of the window (`0.3` / `"30%"`); `0` means size alone decides. |
| `absorb.excludeTools` | string[] | `[]` | 🟢 ACTIVE | Tool names that are never eligible for absorption. |
