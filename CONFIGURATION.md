...
| `compress.nudgeGrowthTokens` | number | window-scaled: `50000` at 150K+; `clamp(window/3, 8000, 50000)` below (#398) | 🟢 ACTIVE | Token growth step for soft compression nudges. |
...
- **Status:** 🟢 ACTIVE
- **Description:** Tool-name patterns **excluded from the soft-protected recent zone** (`preserveRecentMessages` window): matching tool results inside the recent window become compressible immediately instead of aging out first. The kernel built-in keeps `read`/`bash` compressible because they are the largest reclaimable mass — but that same default folds freshly-read files right away in batch-read workflows and can descend into the fold→re-read death loop (upstream billion-context #1198/#1277). **Recommended remedy — remove only `read`:**
...
- **Type:** `number`
- **Default:** window-scaled (#398) — `50000` for windows >= 150K or unknown; otherwise `clamp(window / 3, 8000, 50000)` (a 50K window gets `16667`, a 100K window `33333`)
- **Status:** 🟢 ACTIVE
...
- **Window-scaled default (#398):** when this key is unset, sub-150K windows default the band to a third of the window. The kernel's fixed 50000 only makes sense at 150K+: below that, 50K of growth can never accumulate before the 75% force band takes over, leaving the soft cadence dead weight and compression driven entirely by the forced/emergency bands. With a 50K window the ~16.7K band starts soft-nudging at ~52% usage and re-arms every ~16.7K of growth — small-context sessions stay in a steady compress loop instead of whipsawing into forced compression. An explicit value always wins; set `50000` to restore the legacy fixed band. The same-turn re-inject floor scales with the band as well (#406): the kernel's re-arm gate floors at a fixed 20000 (`minGrowthFloor`), which exceeded the scaled band on sub-50K windows and left an ignored pressure nudge without a same-turn second chance before the 95% emergency band; when this key is unset the extension now sets `minGrowthFloor` to `min(20000, band)`, so on windows under ~60K the gate tracks the band itself (a 20K window re-arms after ~8K of growth). An explicit `nudgeGrowthTokens` leaves the floor untouched.
- **Small-context recipe (~50K windows):** with #398 the compress side needs no knobs. To also absorb oversized single tool outputs early (instead of letting them ride until a compress), merge [PR #208](https://github.com/ranxianglei/billion-context-pi/pull/208) (open at the time of writing — key shape tentative) and set `{ "absorb": { "minToolTokens": 4000 } }` — the default 1000 (2% of a 50K window) also absorbs ordinary outputs; 4000 (~8%) targets only the genuinely large ones.
- **Same-turn re-inject:** within one user turn a nudge injects at most once, but once the context has since grown by a full growth floor (mirroring the kernel's anti-thrashing cadence: `max(minGrowthFloor, minGrowthRatio × adaptiveGrowth)` — 22.5K tokens on windows >= 150K; under window-scaled bands the 20K floor shrinks to the band itself (#406), so on windows under ~60K the gate equals one full band) a fresh reminder re-injects in the same turn (issue #269: a model that ignored a 78% nudge used to stay silent until the 95% emergency truncation). After a successful compress the growth baseline re-anchors to the new (smaller) scale, so post-compress regrowth into the pressure band is not held against the pre-compress peak.
