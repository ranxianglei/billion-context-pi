...
- **Description:** On Pi startup, check the npm registry for a newer version of `billion-context-pi` and auto-install it. Set to `false` to avoid all startup network calls. Can also be disabled via the `ACP_AUTO_UPDATE` environment variable (`ACP_AUTO_UPDATE=0` or `ACP_AUTO_UPDATE=false`), which overrides this setting.
  ...
  - **Channel tracking:** the update channel follows the spec pi recorded at install time (the `billion-context-pi` entry in `~/.pi/agent/npm/package.json`). `*`, ranges, or no version → track the `latest` dist-tag; a dist-tag spec like `npm:billion-context-pi@stable` → track that tag (stable installs never receive `dev` prereleases); an exact version pin → auto-update is skipped entirely. Reinstall with a different spec to switch channels.
