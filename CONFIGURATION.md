...
- **Window-scaled default (#398):** when this key is unset, sub-150K windows default the band to a third of the window. The kernel's fixed 50000 only makes sense at 150K+: below that, 50K of growth can never accumulate before the 75% force band takes over, leaving the soft cadence dead weight and compression driven entirely by the forced/emergency bands. With a 50K window the ~16.7K band starts soft-nudging at ~52% usage and re-arms every ~16.7K of growth — small-context sessions stay in a steady compress loop instead of whipsawing into forced compression. An explicit value always wins; set `50000` to restore the legacy fixed band. Known limitation: the same-turn re-inject floor below stays at its 20K minimum, which can exceed the scaled band itself on sub-50K windows — on very small windows an ignored pressure nudge may not get a same-turn reminder before the 95% emergency band (cross-turn re-arms are unaffected; tracked as a follow-up).
