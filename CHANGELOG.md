...
- **feat(guardrail): 通用同参数工具调用熔断 tool-loop-guard** — 小模型（qwen3.8-27b / IQ3 量化实测）在 ACP 压缩后可能退化为反复发出完全相同的工具调用（如连写同一个调试脚本几十遍）；compress 断路器只覆盖 compress 工具，本护栏补齐所有工具：同一工具+同一参数（canonical JSON，键序无关）连续成功执行 `THRESHOLD=3` 次后，第 4 次起拦截并返回行动引导（分析为何未推进目标→换行动→或改参数），理由携带上次结果摘要（400 字符封顶）；连续拦截 `MAX_CONSECUTIVE_BLOCKS=5` 次后 `terminate` 终止回合防无限烧 token（qwen3.8 实测会无视拦截理由逐字重试）；距上次同名调用 >5min 视为新轮重置计数；`compress` 排除（由 compress 断路器自管）。实机验证：GSQ-RCO-IQ3_S 无人值守 2h/194 轮建站任务中拦截重复 write 后模型正确换路径继续工作。新增 `src/tool-loop-guard.ts`（纯函数核心 `evaluateCall`/`noteResult` + `wireToolLoopGuard`）+ 10 项测试
