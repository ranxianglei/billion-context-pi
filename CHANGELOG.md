...
- **fix(compress): 成功 compress 的每轮节流，堵住“洗记忆续命”死循环（run2 实证）** — 此前断路器只统计失败/no-op compress，`noteCompressOutcomes` 在 success 时反而清零失败计数，成功 compress 完全无节流。实战案例（本地 2h 无人值守长任务，session 01a06693）：模型陷入工具重复循环后被 BLOCKED 反馈拦截，随即主动连续 compress（3 分钟内 3 次：29.2K→9.9K、9.9K→9.9K、10.5K→5.0K）把刚收到的重复调用拒绝反馈连同失败史一起压缩掉，失忆后逐字重发同一调用，靠“洗记忆”无限续命直至进程终止。现在每用户轮累计成功 compress 达上限后拒绝后续调用：`[ACP] THROTTLED — N successful compresses already ran this turn` 面板 + 明确指令（再压会抹掉你自己刚收到的失败反馈——若你正要重复已尝试的动作，换方法而不是压缩遗忘），附当前可压缩 ranges 快照供合法清理；新 user 消息重置预算（正常跨轮压缩不受影响），失败/no-op 不消耗预算；默认上限 `MAX_SUCCESSFUL_COMPRESSES_PER_TURN=4`（给 T2→T3 蒸馏链 3 次留余量，t3-rewrite-guard 场景不误伤），`compress.maxSuccessfulPerTurn` 可配置，0 禁用
