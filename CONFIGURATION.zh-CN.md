...
- **随窗口缩放的默认值（#398）：** 未显式设置时，低于 150K 的窗口默认把增长带设为窗口的三分之一。内核固定的 50000 只在 150K+ 窗口有意义：更小的窗口里，还没攒满 50K 增长，75% 强制带就接管了，软节奏形同虚设，压缩完全被强制/emergency 带驱动。50K 窗口下 ~16.7K 的增长带让软 nudge 在 ~52% 占用即介入，之后每 ~16.7K 增长重新布防——小上下文会话进入平稳的压缩循环，而不是被抽进强制压缩。显式值优先；设 `50000` 可恢复旧固定带。同轮重注入下限随缩放带一起缩放（#406）：内核防抖重布防门槛的下限 `minGrowthFloor` 固定为 20000，在小窗口上会高于缩放带本身——极小窗口里被忽略的压力 nudge 在 95% emergency 带之前收不到同轮重提醒；此键未显式设置时，扩展现在把 `minGrowthFloor` 设为 `min(20000, 带值)`，约 60K 以下窗口的门槛即跟随缩放带（如 20K 窗口增长 ~8K 后重新布防）。显式设置 `nudgeGrowthTokens` 时该下限保持不变。
- **小上下文配方（~50K 窗口）：** #398 之后压缩侧无需任何旋钮。若还想让超大单次工具输出提前吸收（而不是等一次 compress），合并 [PR #208](https://github.com/ranxianglei/billion-context-pi/pull/208)（行文时仍 OPEN——键名以合并版为准）并设置 `{ "absorb": { "minToolTokens": 4000 } }` —— 默认 1000（占 50K 窗口 2%）连普通输出也会吸收；4000（~8%）只针对真正的大输出。
- **同轮重注入：** 同一用户轮内 nudge 至多注入一次，但上下文自上次注入后又增长满一个增长门槛（镜像内核防抖 cadence：`max(minGrowthFloor, minGrowthRatio × adaptiveGrowth)`——窗口 ≥ 150K 时 22.5K token；随窗口缩放的带下 20K 下限收缩为带值本身（#406），约 60K 以下窗口门槛即一个完整增长带）时，会在同轮重新注入新提醒（issue #269：模型忽略 78% nudge 后，原来会一直沉默到 95% emergency 机械截断）。成功 compress 后增长基线重锚到新（更小）刻度，压缩后重新长回压力带不会被压缩前峰值压制。
