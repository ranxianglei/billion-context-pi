...
| `rules` | boolean | `false` | 🟢 ACTIVE | 注册可选的 `acp_rule` 工具（记录 / 列表 / 按 id 删除 / 全部清空）及人类侧 `/acp-rule` 命令：简短、原则性的提醒，硬保护免于压缩。默认关闭。 |
...
`rules` 键控制 `acp_rule` 工具与 `/acp-rule` 命令 —— 一个可选机制，把简短、原则性的提醒持久化在会话里，使其穿越上下文压缩。
...
- **默认值：** `false`
- **状态：** 🟢 ACTIVE
- **说明：** 为 `true` 时，在会话启动时注册 `acp_rule` 工具并启用 `/acp-rule` 人类命令。每次调用仅执行一个操作：传入一条简短的提醒即记录（回显 `Recorded ruleN: …`）；不带参数调用则列出全部已记录规则；传入带规则 id 的 `delete`（如 `"rule3"`）删除该单条规则（`Removed ruleN: <text>`）；传入 `clear: true` 一次性清空全部规则（`Cleared N rule(s).`）。`delete` 与 `clear` 彼此互斥、且与 `rule` 互斥 —— 混合调用会被拒绝且不产生任何变更 —— 因此过时或误记的规则应直接删除，而不是再记一条"作废声明"对冲。参数形状与 [billion-context](https://github.com/ranxianglei/billion-context) 代理提供的 `acp_rule` 逐字节一致，因此在一种宿主下训练的模型在另一种宿主下看到的是同一个工具。规则保存在会话 ACP 状态旁路文件中（重启后保留），并**硬保护免于压缩** —— 即使周围内容全部被压缩，其工具调用与结果仍保持可见。不涉及系统提示词：使用引导完全写在工具描述里，每轮不做任何重新注入。校验错误（空/超长/重复/超限/未知 id）原样返回给模型。默认上限：最多 50 条 × 每条 300 字符。

  > **如果你同时运行了启用规则的 billion-context 薄插件**，两个插件都会在 pi 中注册一个 `acp_rule` 工具；pi 按名称对工具去重（同名以首次注册为准，另一个被静默忽略），且两个实现背后是不同的存储。请只在一侧启用规则 —— 详见 README 的 *`/acp-rule` 命令* 一节。
