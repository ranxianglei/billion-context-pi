...
> **优先级：** 环境变量 &gt; 项目文件 &gt; 全局文件 &gt; 内置默认值。同一范围内，新旧两个位置都存在且新位置可解析时，新位置（agent 目录）优先。

文件在会话启动时加载。缺失文件和未知键会被忽略；格式错误的 JSON 会尽量修复（BOM 头、未加引号的键、尾逗号），无法修复时给出带诊断信息的醒目警告并跳过——扩展绝不会因为配置问题而无法启动。只有文档中列出的键会被读取，其余一律丢弃。

> **旧位置（向后兼容）：** `~/.pi/acp.json`（全局）和 `<项目>/.pi/acp.json`（项目）在新位置不存在时仍会被读取，因此现有配置无需改动即可继续生效。要迁移现有配置，把文件复制到新位置即可（例如 `cp ~/.pi/acp.json ~/.pi/agent/acp.json`），新位置随后会优先生效。不会自动写入任何文件。若新位置的文件存在但无法解析，会给出醒目警告，并在该范围内回退使用旧位置文件——损坏的副本永远不会遮蔽可用的配置。
